package com.mp.diffiehellmanserver;

/**
 * Matthew Park
 * 7/5/19
 * Server Application for the Diffie-Hellman encryption chat server. Coordinates
 * GUI and socket connection for the server side. 
 * 
 */
...
import com.mp.utilities.EncryptedServerSocketManager;
import com.mp.utilities.GUIManager;
import com.mp.utilities.ServerSocketManager;
import com.mp.utilities.SocketManager;

public class ChatServer implements CommsReceiver, ActionListener{


	private CommsManager manager;
	private XORCipher encoder;
	
	
	/*
	 * Initializes main components of server side.
	 */
	public ChatServer() {
		manager = new EncryptedServerSocketManager();
		manager.setReceiver(this);
		
		GUIManager.input.addActionListener(this);
		GUIManager.fileButton.addActionListener(this);
		
		GUIManager.addText("Waiting for client...");
		
		manager.init();
		
		GUIManager.addText("Client connected!");
